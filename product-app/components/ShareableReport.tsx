...
// FIX: Added missing import for SolutionScoringCanvas.
import { 
    StrategyCanvas,
...
const ShareableReport = React.forwardRef<HTMLDivElement, ShareableReportProps>(
    ({ challenge, report }, ref) => {
        const {
            ...
            valueProposition, leanCanvas, storyboard, financialPlan, investmentMemo,
            riskAnalysis, technicalBlueprint, goToMarket, pitchDeck, strategy,
            redTeam, ethicsReport, ideaArea, targetMarket, userSolution, successScore
        } = report;
            ...
            <div style={{ position: 'absolute', left: '-9999px', top: 0, background: 'white', color: 'black' }} className="w-[900px]">
                <div ref={ref}>
                    ...
                    <main className="max-w-4xl mx-auto p-8">
                        ...
                        {(founderDna || talentStrategy || founders?.some(f => f.analysisProfile)) && (
                            <WebSection title="Founder & Team Strategy">
                                ...
                                {founders?.map(f => f.analysisProfile && (
                                    <div key={f.id} className="mt-8 pt-8 border-t border-gray-200">
                                        <FounderAnalysisProfileCanvas profile={f.analysisProfile} isPrintable={true} />
                                    </div>
                                ))}
